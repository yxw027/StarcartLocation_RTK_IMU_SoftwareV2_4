...
public:
	...
	CGLV(double Re=6378137.0, double f=(1.0/298.257), double wie0=7.2921151467e-5, double g0=9.7883267714);
};
...
public:
	...
	CVect3 phim, dvbm, wm_1, vm_1;//补偿后等效旋转矢量 比力增量 保存前一次角/速度增量采样
	CIMU(void);
...
public:
	...
	void TimeUpdate(double kfts, int fback=1);	// time update 默认fback=1
	void MeasUpdate(double fading=0.1);			// measurement update fading遗忘滤波参数
